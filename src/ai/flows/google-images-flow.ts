 ...
 *
 * - getGoogleImages: 특정 키워드와 관련된 이미지를 검색하는 공개 함수입니다.
...
// 입력 스키마 정의: 'keyword'와 'start' 인덱스를 필드로 가집니다.
const GoogleImagesInputSchema = z.object({
  query: z.string().describe('The keyword to search for images.'),
  start: z.number().optional().describe('The starting index for the search results.'),
});
export type GoogleImagesInput = z.infer<typeof GoogleImagesInputSchema>;

// 개별 이미지 결과의 스키마를 정의합니다.
...
// 출력 스키마 정의: 이미지 결과 배열과 다음 페이지 인덱스를 포함합니다.
...
/**
 * Google Custom Search API를 호출하여 이미지를 가져오는 핵심 Genkit 플로우입니다.
 */
